...
const SPECIFIC_PERSON_PATTERNS = [
  // Name patterns - "Find {Name}" BUT NOT followed by business keywords
  { pattern: /\b(?:find|looking\s+for)\s+([A-Z][a-z]+)\s+(?:from|at|with)\b/gi, weight: 1.0 },
  { pattern: /\bwho\s+is\s+[A-Z][a-z]+(?:\s+[A-Z]\.?)?\s*(?:[A-Z][a-z]+)?\s*\??$/gi, weight: 1.0 }, // "Who is {Name}?" at end of query

  // "{Name} from/at/with {Company}" - strong person indicator
  { pattern: /\b[A-Z][a-z]+\s+(?:from|at|with)\s+[A-Z]/gi, weight: 0.95 },
...
export function classifyIntent(query: string): IntentResult {
  ...
  // NOTE: Use original query for person patterns (needs capital letters for names)
  ...
  const specificPersonMatch = calculatePatternScore(trimmedQuery, SPECIFIC_PERSON_PATTERNS);
