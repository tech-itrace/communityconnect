...
export async function getMemberStats(): Promise<MemberStats> {
    ...
    const statsQuery = `
        SELECT 
            ...
            AVG(annual_turnover) FILTER (WHERE annual_turnover IS NOT NULL) as avg_turnover
        FROM community_members
    `;
    ...
    // Get top skills (extract from skills field)
    ...
    const memberStats: MemberStats = {
        ...
        averageTurnover: parseFloat(stats.avg_turnover) || 0,
        topSkills,
        topCities
    };
...
export async function getUniqueServices(): Promise<string[]> {
    const queryText = `
        WITH services_split AS (
            SELECT TRIM(unnest(string_to_array(products_services, ','))) as service
            FROM community_members
            WHERE products_services IS NOT NULL AND products_services != ''
        )
